...
CHECKPOINT_FILE = os.path.join(CHECKPOINT_FOLDER, "checkpoint_03_31_halfs.pkl")
...
def main():
    # Set up logger
    log = setup_logger("Lee_ml_pipeline_run", log_file=os.path.join("logs", "Lee_full_dataset_ml_halfs.log"))
    dataset_no = 20
    paradigm = "MI"
        ...
        for subject in range(start_subject, 55):
            ...
            for run in range(start_run, 3):
                try:
                    ...
                    n_epochs = len(epochs)
                    epochs_fh = epochs[: n_epochs // 2]
                    epochs_sh = epochs[n_epochs // 2:]

                    log.info(f"{subject}-{run} croped in halfs")

                    for epochs, indx in zip([epochs_fh, epochs_sh],['fh', 'sh']):
                        ...
                        directory = os.path.join(os.getcwd(), 'data', 'results', 'pipeline_scores', 'lee2019', '2025-03-31_halfs')
                        os.makedirs(directory, exist_ok=True)
                        current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

                        file_name = os.path.join(directory, f'results_{subject:02}-{run:02}_{indx}_{current_time}.pkl')
