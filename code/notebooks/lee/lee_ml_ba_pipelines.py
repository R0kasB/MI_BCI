...
CHECKPOINT_FILE = os.path.join(CHECKPOINT_FOLDER, "checkpoint_03_30.pkl")
            ...
            for run in range(start_run, 3):
                try:
                    data = load_procesed_data(dataset_no, paradigm, subject, run, include=['epochs_raw'], 
                                              extra_mark='(-1_5)(8_30Hz)')
                    epochs = data["epochs_raw"]

                    results = train_and_evaluate(epochs, n_splits=5, log=log, save_model=True, 
                                                subject=subject, run=run,
                                                construct_pipelines=True,
                                                feature_pipelines= features,classifier_pipelines=classifiers,
                                                n_jobs=-1)

                    directory = os.path.join(os.getcwd(), 'data', 'results', 'pipeline_scores', 'lee2019', '2025-04-16')
