...
CHECKPOINT_FILE = os.path.join(CHECKPOINT_FOLDER, "checkpoint_04-04-autorej.pkl")
...
def main():
    # Set up logger
    log = setup_logger("Lee_ml_pipeline_run", log_file=os.path.join("logs", "Lee_full_dataset_ml_run_autorej.log"))
    dataset_no = 20
    paradigm = "MI"
    ...
    try:
        for subject in range(start_subject, 55):
            ...
            for run in range(start_run, 3):
                try:
                    data = load_procesed_data(dataset_no, paradigm, subject, run, include=['epochs_raw_autoreject'])
                    ...
                    directory = os.path.join(os.getcwd(), 'data', 'results', 'pipeline_scores', 'lee2019', '2025-04-04_autorej')
